...
def parse(engine):
    log_files = list(Path('./logs/').glob("*.log.gz")) + [Path("./logs/latest.log")]
    for file in list(log_files):
        if file.name == "latest.log":
            ...
            year, month, day = [
                int(x) for x in file_name_parser.match(file.name).group(1, 2, 3)]
        if file.name.endswith("gz"):
            with open(file, "rb") as data_file:
                try:
                    log_data = gzip.decompress(data_file.read()).decode("utf-8")
                except EOFError:
                    print("warning: could not open", file, "it is most likely corrupted")
                    continue
        else:
                ...
                if re.match(r"^User Authenticator #\d+/INFO$", source):
                    ...
                    if not match:
                        ...
                        if match[0].username != username:
                            past_usernames = json.loads(match[0].past_usernames) + [username]
                            match[0].past_usernames = json.dumps(past_usernames)
                elif source == "Server thread/INFO":
                    if join_message_match := re.match(r"^(.*) joined the game$", message):
                        player_uuid = usernames[join_message_match.group(1)]
                        open_sessions[player_uuid] = timestamp
                    elif leave_message_match := re.match(r"^(.*) left the game$", message):
                        ...
                        session.add(PlaySession(start_time=start_time,
                                    end_time=timestamp, user_id=player.id))
                    elif villager_died_message_match := re.match(r"^Villager .*?\[(.*?)\] died, message: '(.*?)'$", message):
                        death_data, death_message = villager_died_message_match.group(1, 2)
                        session.add(
                            VillagerDeath(
                                time=timestamp,
                                had_profession=(not death_message.startswith("Villager")),
                                villager_data=death_data, message=death_message))
                    elif chat_message_match := re.match(r"^<(.*?)> (.*)$", message):
                        ...
                        session.add(UserDeath(time=timestamp, user=dier, message=message))
                session.commit()
